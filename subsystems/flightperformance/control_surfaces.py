...
def aileron_sizing(params):
    ...
    for i in range(100):
        ...
        bank_angle = 30                                         # Desired bank angle in degrees (deg) time for roll authority = 2/3s 45deg/s for our UAV
        ...
        C_l_delta_a = (2 * c_l_alpha * tau * I_1) / (S_ref * b)             # Aileron control derivative
        C_l_p = (-4 * (c_l_alpha + c_d_0) * I_2) / (S_ref * b**2)           # Roll damping derivative
        P = (-1 * C_l_delta_a * delta_a * 2 * V_stall) / (C_l_p * b)        # Aircraft steady state roll rate
        delta_t = (bank_angle * np.pi) / (180 * P)                          # Required time to achieve bank angle (delta_t)
