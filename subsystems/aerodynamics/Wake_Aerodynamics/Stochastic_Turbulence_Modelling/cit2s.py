...
if __name__ == "__main__":
    ...
    Kt = -0.21; Kq = -3      # gains on "theta" and "q"
    K  = numpy.array([0, 0, Kt, Kq, 0, 0, 0]) # feedback matrix
    At = A-B[:,0]*K;         # new A matrix = (A - BK) because of feedback
    
    # Save variables to file
    save_vars = {"A": A, "At": At, "B": B,
                "sigmaug_V": sigmaug_V,
                "sigmaag": sigmaag, "Lg": Lg, "V": V, "c": c}
    
    file = open("dumfile.txt", 'wb')
    pickle.dump(save_vars, file)
    file.close()
        
    # Load variables from file
    with open('dumfile.txt', 'rb') as f:
