...
def get_aircraft_config_aerodynamics_pd(aircraft_type_pd, config_name="clean", aspect_ratio=9.0):
    ...
    """
    # Base clean values from PDF2 Page 12 for Business Jet example (used as baseline)
    # These are C_D0 and e for the *basic clean airframe*. Flaps/gear are additive.
    base_C_D0_clean_jet = 0.0145
    base_e_clean_jet = 0.85
    
    # C_Lmax values from PDF2 Page 9 (Typical values for Business Jet)
    # For stall constraints, we use specific example values if given, or typical ones.
    C_Lmax_clean_typical_jet_min, C_Lmax_clean_typical_jet_max = 1.4, 1.8
    C_Lmax_TO_typical_jet_min, C_Lmax_TO_typical_jet_max = 1.6, 2.2
    C_Lmax_L_typical_jet_min, C_Lmax_L_typical_jet_max = 1.6, 2.6

    # Default to example values from PDF2 P12 for "Jet"
    C_D0, e_val, C_Lmax = base_C_D0_clean_jet, base_e_clean_jet, C_Lmax_clean_typical_jet_max # Default

    if "uav" in aircraft_type_pd:
        # Values from PDF2 Page 12 for "Jet" example table:
        if config_name == "clean_config_P12": # Explicitly the "Clean*" from table
            ...
            C_D0, e_val, C_Lmax = base_C_D0_clean_jet, base_e_clean_jet, 1.8
        elif config_name == "stall_landing_P21": # For reproducing PDF2 Page 21 stall lines
            C_D0, e_val, C_Lmax = base_C_D0_clean_jet, base_e_clean_jet, 2.6 # Using example C_Lmax for this specific config for stall constraint
        ...
        # General approach using corrections from PDF2 Page 11 (if base is truly clean)
            ...
            C_D0 = base_C_D0_clean_jet + 0.015 # Avg of 0.01-0.02
            e_val = base_e_clean_jet + 0.05
            C_Lmax = C_Lmax_TO_typical_jet_max # e.g., 2.2
        elif config_name == "generic_landing_flaps_gear_down":
            C_D0 = base_C_D0_clean_jet + 0.065 + 0.020 # Avg Landing flaps + Avg Undercarriage
            e_val = base_e_clean_jet + 0.10 # Landing flaps effect on e
            C_Lmax = C_Lmax_L_typical_jet_max # e.g., 2.6
        else: # Default to base clean if specific config not matched for detailed table
             C_D0, e_val, C_Lmax = base_C_D0_clean_jet, base_e_clean_jet, C_Lmax_clean_typical_jet_max
...
def interpolate_TOP_pd(S_TO_ft, ac_category="jet_2_engine"): # [cite: 44] (chart interpretation)
    chart_S_TO_ft = np.array([2000, 3000, 4000, 5000, 6000, 7000, 8000, 9000, 10000]) # Extended range
    if ac_category == "jet_2_engine": # "JET OVER 50ft" (2 engines)
...
def constraint_take_off_distance_jet_pd(wing_loading_range_Npm2, S_TO_req_m, C_Lmax_TO, sigma_TO=1.0, ac_category_TOP_chart="jet_2_engine"): # [cite: 51, 52]
    if C_Lmax_TO <= 0 or sigma_TO <=0 : return np.full_like(wing_loading_range_Npm2, np.nan)
    S_TO_req_ft = m_to_ft(S_TO_req_m)
    TOP_jet_Npm2 = interpolate_TOP_pd(S_TO_req_ft, ac_category_TOP_chart)
    if TOP_jet_Npm2 <= 0: return np.full_like(wing_loading_range_Npm2, np.nan)
    C_L_TO = C_Lmax_TO / (1.1**2) # [cite: 49]
    T_W = wing_loading_range_Npm2 / (TOP_jet_Npm2 * C_L_TO * sigma_TO)
    return T_W
...
def constraint_landing_distance_pd(wing_loading_range_Npm2, S_L_req_m, C_Lmax_L, W_L_over_W_TO, rho_kg_m3, ac_type="CS25"): # [cite: 77, 86]
    ...
    factor = 0.5847 if "CS25" in ac_type or "jet" in ac_type else 0.5915 # [cite: 76, 86]
...
def constraint_cruise_speed_jet_pd(wing_loading_range_Npm2, V_cruise_ms, alt_cruise_m,
...
def constraint_climb_rate_jet_pd(wing_loading_range_Npm2, roc_ms, alt_climb_m,
...
def constraint_climb_gradient_jet_pd(wing_loading_range_Npm2, grad_req, alt_grad_m,
