...
def wing_weight_N(params: DesignParameters):
    ...
    W_wing_lb = 0.00125 * (N_to_lbf(params.weight.W_TO))*(m_to_ft(params.wing.b_w)/math.cos(params.wing.Lambda_w_quarter))**0.75 * (1+(6.3*math.cos(params.wing.Lambda_w_quarter)/m_to_ft(params.wing.b_w))**0.5)* params.max_load_factor**0.55*(m_to_ft(params.wing.S_w)*m_to_ft(params.wing.b_w)/(m_to_ft(params.wing.t_r)*params.weight.W_TO*math.cos(params.wing.Lambda_w_quarter)))**0.3
