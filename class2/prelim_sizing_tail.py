...
if __name__ == "__main__":
    ...
    Mach_cruise = params.cruise_mach + 0.05 # Adjusted for tail
    Mach_cross = 0.935
    Lambda_025c_t = calculate_sweep_angle_025c_rad(Mach_cruise, Mach_cross)
    taper_ratio_t = calculate_taper_ratio(Lambda_025c_t)

    b_t = math.sqrt(S_t * params.empennage.A_t)
    
    c_root_t, c_tip_t = calculate_chord_lengths(S_t, b_t, taper_ratio_t)
    MAC_t, y_LEMAC_t = calculate_MAC_and_y_LEMAC(c_root_t, c_tip_t, b_t)
    
    h = params.cruise_altitude  # Altitude in meters
    W_TO = params.weight.W_TO  # Takeoff weight in Newtons
    
    Lambda_LE_t = calculate_sweep_angle_LE(Lambda_025c_t, c_root_t, b_t, taper_ratio_t)
    Lambda_05c_t = calculate_sweep_angle_x_c(Lambda_LE_t, c_root_t, b_t, 0.5, taper_ratio_t)
    t_c_t = calculate_thickness_ratio(h, Mach_cruise, W_TO, S_t, Lambda_05c_t, Mach_cross)
    
    dihedral_angle_t_rad = calculate_dihedral_angle_rad(Lambda_025c_t)
